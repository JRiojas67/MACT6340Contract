...
describe("JRiojasIronMindCreationsNFTContract", async function () {
  ...
  describe("construction and initialization", async function () {
    ...
    it("should be have symbol JRIMC", async function () {
  ...
  describe("mintTo function", async function () {
    ...
    it("should revert if called with no ether", async function () {
      const [owner, artist, buyer] = await ethers.getSigners();
      expect(
        JRiojasIronMindCreationsNFTContract.connect(buyer).mintTo(args.base_uri, {
    ...
    it("should have a totalSupply = 1 with after first mint", async function () {
      ...
      const mint1 = await JRiojasIronMindCreationsNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint1.wait(1);
      const currentValue = await JRiojasIronMindCreationsNFTContract.totalSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should increase the totalSupply by 1 with with each mint", async function () {
      ...
      mint1.wait(1);
      const mint2 = await JRiojasIronMindCreationsNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint2.wait(1);
    ...
    it("should emit an event when minting is completed", async function () {
      ...
      expect(
        ...
        .to.emit(JRiojasIronMindCreationsNFTContract, "MintingCompleted")
        .withArgs(1, owner);
    });
    ...
    it("should have set the token uri during minting function", async function () {
      ...
      mint1.wait(1);
      const currentValue = await JRiojasIronMindCreationsNFTContract.tokenURI(1);
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should emit an event after funds are distributed during mint", async function () {
      ...
      const uriString = "someString";
      expect(
        JRiojasIronMindCreationsNFTContract.connect(buyer).mintTo(uriString, {
          value: args.mint_price,
        })
      )
        .to.emit(JRiojasIronMindCreationsNFTContract, "FundsDistributed")
